...
import { JIKAN_API, CACHE_CONFIG, RATE_LIMIT } from "@/shared/constants/api";
import { getSfwParam } from "@/shared/api/cookies";
import { FetchError, PaginationData, ApiResponse, FetchParams, CacheConfig, JikanApiErrorResponse } from "@/shared/types/api";
...
async function fetchJson<T>(url: string, cacheConfig: CacheConfig, timeoutMs = 30000): Promise<T> {
	...
	try {
		...
		if (!res.ok) {
			...
			try { details = await res.json() as JikanApiErrorResponse; } catch { }
...
export async function fetchList<T>(
	endpoint: string,
	params: FetchParams & { page: number; limit: number } = { page: 1, limit: 24 },
	...
	return {
		...
		pagination: {
			...
			has_next_page: Boolean(lastPagination?.has_next_page ?? firstPagination?.has_next_page),
			last_visible_page: lastPagination?.last_visible_page ?? firstPagination?.last_visible_page ?? 1,
			items: {
